...
Foundation models now exist for both modalities: EchoJEPA, a video Joint-Embedding Predictive Architecture for echocardiography, and ECG-FM, a wav2vec2-style self-supervised model for the 12-lead ECG \citep{echojepa,ecgfm}. EchoingECG is the closest point of comparison, since it also uses paired ECG and echocardiography data to predict echocardiography-derived cardiac function labels, including reduced LVEF \citep{echoingecg}. Its objective, however, is distinct from ours: EchoingECG trains an uncertainty-aware ECG model by using ECHO-CLIP as a teacher, thereby distilling echocardiographic information into ECG representations. In contrast, we do not introduce or fine-tune a new ECG encoder. We use EchoJEPA and ECG-FM as fixed feature extractors, then study what their pretrained representations can support when combined with lightweight downstream models for LVEF estimation. This shifts the emphasis from learning a new cross-modal ECG representation to evaluating how existing echo and ECG foundation-model embeddings behave in a more realistic deployment setting, where one modality may be unavailable at inference and errors may differ across patient subgroups. In that sense, EchoingECG and our study are complementary. EchoingECG uses echocardiography to improve what can be inferred from ECG alone, whereas we ask what is gained and lost when frozen ECG and echo embeddings are used together for LVEF estimation.
