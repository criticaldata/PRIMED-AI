...
def main() -> None:
    ...
    n = res["n"]
    print(
        f"n train/val/test = {n['train']}/{n['val']}/{n['test']} | "
    ...
    for nm in ("val", "test"):
        m = res["splits"][nm]
        print(
            f"{nm:5} baseMAE {m['baseline_mae']:.2f} | ridgeMAE {m['ridge_mae']:.2f} | "
            f"EF40 AUROC reg {m['ef40_auroc_from_regression']:.3f} "
            f"logreg {m['ef40_auroc_logreg']:.3f}"
        )
    t = res["splits"]["test"]
    print(
        f"TEST MAE {t['ridge_mae']} CI {t['ridge_mae_ci95']} (baseline {t['baseline_mae']}) | "
