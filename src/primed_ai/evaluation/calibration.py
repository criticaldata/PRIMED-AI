...
def platt_probabilities(scores: np.ndarray, labels: np.ndarray) -> np.ndarray:
    ...
    learns the sign and slope. Note: an in-sample fit gives an optimistic ECE; a held-out
    calibration split is preferred when data permit.
    """
...
def run_calibration(
    ...
    # Lower predicted LVEF => higher P(EF<=40); Platt learns the mapping.
    prob = platt_probabilities(-np.asarray(preds["prediction"], dtype=np.float64), labels)
    ...
    result = {
        ...
        "figure": str(figure),
        "note": "In-sample Platt scaling; use a held-out calibration split for a deployment estimate.",
    }
